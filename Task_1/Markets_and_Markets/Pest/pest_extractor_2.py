...
df = pd.read_csv('pest_helper_file.csv')
...
class Chemical_Extractor:
    ...
    def preprocessing(self):
        ...
        rel_tags = related_section.findAll(['h3', 'table', 'ul', 'p', 'h2'], recursive = False)
    ...
    def extraction(self, rel_tags):
        ...
        for tags in rel_tags:
            ...
            if(got_table):
                ...
                #print("-----------------------------\n")
                #print(indicator_symbol)
                if(indicator_symbol == 'h3'):
                    #print(str(tags.text).split())
        ...
        for tags in rel_tags:
            ...
            if(got_table):
                if(indicator_symbol == 'h3'):
                    if('Region' in str(tags.text).split()):
                        break
                    else:
                        counter += 1
                if(indicator_symbol == 'ul'):
                    ...
                    for lis in all_lis:
                        ul = lis.find('ul')
                        if(ul):
                            self.level[level_counter] = ul.text
                            level_counter += 1
                    self.subsegments[counter] = ", ".join([all_lis[i].find(text=True, recursive = False) for i in range(len(all_lis))])
